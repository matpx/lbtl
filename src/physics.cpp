  ...
  world::main.observer<comps::Transform, comps::RigidBody>()
      ...
      .each([](comps::Transform &transform, comps::RigidBody &rigidbody) {
        ...
        rigidbody._rigidbody = world->createRigidBody(reactphysics3d::Transform(
            ...
            reactphysics3d::Quaternion(comp_rotation.X, comp_rotation.Y, comp_rotation.Z,
                                       comp_rotation.W)));
...
void update(const float delta_time) {
  ...
  world::main.query_transform_rigidbody.each([](comps::Transform &transform,
                                                comps::RigidBody &rigidbody) {
    const reactphysics3d::Vector3 &react_position =
        rigidbody._rigidbody->getTransform().getPosition();
    const reactphysics3d::Quaternion &react_orientation =
        rigidbody._rigidbody->getTransform().getOrientation();

    transform.translation = HMM_V3(react_position.x, react_position.y, react_position.z);
    transform.rotation =
        HMM_Q(react_orientation.x, react_orientation.y, react_orientation.z, react_orientation.w);
  });
}
