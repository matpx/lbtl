...
inline void HMM_TranslateInplace(HMM_Mat4 &Left, HMM_Vec3 Translation) {
  Left.Elements[3][0] += Translation.X;
  Left.Elements[3][1] += Translation.Y;
  Left.Elements[3][2] += Translation.Z;
}

inline HMM_Quat HMM_QuaternionMultiplyVec3(HMM_Quat q, HMM_Vec3 v) {
  HMM_Quat Result;
...
inline HMM_Vec3 HMM_RotateVec3(HMM_Quat q, HMM_Vec3 v) {
  HMM_Quat inverted;
  inverted.X = -q.X;
  inverted.Y = -q.Y;
  inverted.Z = -q.Z;
...
inline HMM_Vec3 operator*(HMM_Quat Left, HMM_Vec3 Right) { return HMM_RotateVec3(Left, Right); }
